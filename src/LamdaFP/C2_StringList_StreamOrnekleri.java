...
public class C2_StringList_StreamOrnekleri {
    ...
    public static List<String> dcIleBasYaz2(List<String> l) {
        return l.stream().filter(t -> t.startsWith("d")).filter(t -> t.startsWith("c")).
    ...
    //S3: alfabeye  gore siralayalim list olarak
    ...
    public static void ilkBuyukDigerKucuk(List<String> l) {
        l.stream().map(t -> t.substring(0, 1).toUpperCase() + t.substring(1).
                toLowerCase()).forEach(t -> System.out.println(t));
    }
