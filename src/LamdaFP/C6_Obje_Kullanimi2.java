...
public class C6_Obje_Kullanimi2 {
    public static void main(String[] args) {
        ...
        Ogretmen ogretmen3 = new Ogretmen("Alim", "Sana", "Bilgisayar", 35, 10);
...
yasSirala(list);
    ...
    public static List<Ogretmen> otuzdanBuyuk(List<Ogretmen> l) {
        l.stream().filter(t -> t.getYas() > 30).
                forEach(t -> System.out.printf("%-16S %-14S %-16S %-5d %-5d \n",
                        t.getOgretmenADI(), t.getOgretmenSoyAdi(),
                        t.getBransi(), t.getYas(), t.getCalismaSuresi()));
    ...
    // S3 : isimlerini alfabetik siraya gore sirala
    public static void sirala(List<Ogretmen> l) {
        l.stream().sorted(Comparator.comparing(t -> t.getOgretmenADI())).forEach(Methods::yazPrintF);
    }
    ...
    public static void yasSirala(List<Ogretmen> l) {
        l.stream().peek(System.out::println).sorted(Comparator.comparing(Ogretmen::getYas).reversed()).
